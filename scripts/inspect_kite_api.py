    ...
    for cat in ("quantity", "collateral", "margin", "pnl"):
        ...
        if cat == "quantity":
            rlines += [
                "",
                "=" * 72,
                "SEMANTIC NOTES (verified against live Zerodha API v3):",
                "-" * 72,
                "  holdings[].quantity             — free/tradeable (can sell today); excludes T1, pledged",
                "  holdings[].t1_quantity           — T+1 unsettled (bought recently, settling tomorrow)",
                "  holdings[].authorised_quantity   — pledge initiated, CDSL OTP not yet done",
                "                                     shares are frozen here, not in any other field",
                "  holdings[].collateral_quantity   — pledge APPROVED and active (v3 canonical field)",
                "                                     LIVE VERIFIED: fully-pledged accounts ONLY have",
                "                                     this field > 0; quantity/used_quantity both 0",
                "  holdings[].used_quantity         — legacy or active-margin-in-use indicator",
                "                                     may be 0 even when shares are fully pledged",
                "                                     may equal collateral_quantity on older API",
                "  holdings[].opening_quantity      — balance at session open (STALE intraday)",
                ...
                "    [+ opening_quantity as last-resort fallback if all above = 0]",
                "",
                ...
                "",
                "  DO NOT use opening_quantity as primary — it does not update intraday.",
            ]
        elif cat == "collateral":
            rlines += [
                "",
